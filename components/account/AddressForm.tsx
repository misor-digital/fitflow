...
export default function AddressForm({ mode, initialData, onSuccess, onCancel }: AddressFormProps) {
  ...
  const handleMethodChange = useCallback(
    (method: DeliveryMethod) => {
      ...
      if (method === 'address') {
        // Switching to home delivery: clear speedy office
        setSelectedOffice(null);
      } else {
        // Switching to speedy office: clear home address fields
        setAddress((prev) => ({
          ...prev,
          city: '',
          postalCode: '',
  ...
  // Office selected from Speedy widget
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const result =
      deliveryMethod === 'address'
        ...
        : validateSpeedyOffice(address, selectedOffice);
    ...
    const body =
      deliveryMethod === 'address'
        ...
        : {
            ...
            speedyOfficeAddress: selectedOffice!.address,
            deliveryNotes: address.deliveryNotes,
            isDefault: address.isDefault,
          };
    ...
    try {
      const url =
        mode === 'edit' && initialData ? `/api/address/${initialData.id}` : '/api/address';
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-3 gap-3">
            ...
            <div>
              ...
              <input
                ...
                className={`w-full border rounded-lg px-3 py-2 text-sm focus:ring-[var(--color-brand-orange)] focus:border-[var(--color-brand-orange)] outline-none ${
                  errorFor('postalCode') ? 'border-red-500' : ''
                }`}
              />
              {errorFor('postalCode') && (
          ...
          <div>
            ...
            <input
              ...
              className={`w-full border rounded-lg px-3 py-2 text-sm focus:ring-[var(--color-brand-orange)] focus:border-[var(--color-brand-orange)] outline-none ${
                errorFor('streetAddress') ? 'border-red-500' : ''
              }`}
            />
          ...
          {/* Entrance + Floor + Apartment Row */}
          <div className="grid grid-cols-3 gap-3">
            <div>
              <label
                htmlFor={`${uid}-buildingEntrance`}
                className="block text-sm font-medium text-gray-700 mb-1"
              >
                Вход
              </label>
